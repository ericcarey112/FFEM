    ...
    public class InputFile
    {
            ...
            if (File.Exists(FilePath))
            {
                ...
                for (int i = 0; i < assembly.NumberOfElements; i++)
                {
                    ...
                    if (type == "4NT")
                    {
                        ...
                        {
                            element.Model = new IsotropicLinearElastic(element.NumIPs, 0, fibers.E2, fibers.v23);
                        }
                        // Transversly isotropic linear elastic constitutive model
                        else if (element.ModelID == 2)
                        {
                            element.Model = new TransverslyIsotropicLinearElastic(element.NumIPs, 0, fibers.E1, fibers.E2, fibers.v23, fibers.G23);
                        }
                    ...
                    else if (type == "3NT" || type == "6NT" || type == "6NQ" || type == "8NQ")
                    {
                        ...
                        if (element.ModelID == 1)
                        {
                            element.Model = new IsotropicLinearElastic(element.NumIPs, 0, matrix.E2, matrix.v23);
                        }
                        // Smeared crack constitutive model
                        else if (element.ModelID == 3)
                        {
                            element.Model = new SmearedCrack(element.NumIPs, 3, matrix.E2, matrix.v23, matrix.Strength, matrix.GIC);
                        }
                    }
        ...
        private static PeriodicBCs ReadBCData(string line, PeriodicBCs BCs)
        {
            ...
            if (beforeColon == "LoadStep")
            {
                ...
                {
                    double[,] InitLoad = new double[2, 2];
        ...
        private static Solver ReadSolverData(string line, Solver solver)
        {
            ...
            if (beforeColon == "ConvergenceTolerance")
            {
                if (double.TryParse(afterColon, out double ConvergenceTolerance))
                {
                    solver.ConvergenceTolerance = ConvergenceTolerance;
                }
                else
                {
                    throw new Exception("Error in reading ConvergenceTolerance in InputOutput --> ReadSolverData");
                }
            }
            return solver;
        }

...
namespace FEMAssembly
{
    ...
    public class ParaviewFiles
    {
        ...
        {
            // Inputs
            int NumElements = assembly.NumberOfElements;
            int NumNodes = assembly.NumberOfNodes;
            ...
            using (StreamWriter writer = new StreamWriter(FilePath))
            {
                ...
                // POINTS
                writer.WriteLine("POINTS " + NumNodes + " float");
                ...
                WritePointData(writer, NumNodes, this.NodalDisplacements);
                writer.WriteLine();

                // Strains
                writer.WriteLine("SCALARS " + "E " + "float64 " + 3);
                writer.WriteLine("LOOKUP_TABLE " + "default");
                WritePointData(writer, NumNodes, this.Strain);
                writer.WriteLine();

                // Stresses
                writer.WriteLine("SCALARS " + "S " + "float64 " + 3);
                writer.WriteLine("LOOKUP_TABLE " + "default");
                WritePointData(writer, NumNodes, this.Stress);
                writer.WriteLine();

                // Principal Strains
                writer.WriteLine("SCALARS " + "PE " + "float64 " + 3);
                writer.WriteLine("LOOKUP_TABLE " + "default");
                WritePointData(writer, NumNodes, this.PrincipalStrain);
                writer.WriteLine();

                // Principal Stresses
                writer.WriteLine("SCALARS " + "PS " + "float64 " + 3);
                writer.WriteLine("LOOKUP_TABLE " + "default");
                WritePointData(writer, NumNodes, this.PrincipalStress);
            }
        }
        ...
        private void GenerateCellIPDataVTKFile(Assembly assembly, int LoadStep)
        {
            ...
            List<double[]> NodalLocations = this.NodalLocations;
            List<int> QuadElements = this.QuadElements;
            int NumQuads = QuadElements.Count;
            int NumNodes = NodalLocations.Count;
            List<int[]> ConnectMat = this.Connectivity;
            ...
            using (StreamWriter writer = new StreamWriter(FilePath))
            {
                ...
                int TotalCellData = NumCells + NumLocalNodes;
                writer.WriteLine("CELLS " + NumCells + " " + TotalCellData);
                ...
                // CELL TYPES (Paraview IDs)
                int CellID = 0;
                writer.WriteLine("CELL_TYPES " + NumCells);
                for (int i = 0; i < NumCells; i++)
                {
                    if (i < NumElements)
                    {
                ...
                // CELL DATA
                writer.WriteLine("CELL_DATA " + NumCells);
                ...
                for (int i = 0; i < NumElements; i++)
                {
                    double damage = 0.0;
                    string type = assembly.ElementList[i].Type;
                    if (type == "3NT" || type == "6NT")
                    {
                        damage = assembly.ElementList[i].Model.StateVars[0][1];
                    }
                ...
                for (int i = 0; i < NumQuads; i++)
                {
                    ...
                    {
                        if (assembly.ElementList[Quad - 1].ModelID == 2)
                        {
                            double damage = assembly.ElementList[Quad - 1].Model.StateVars[j][1];
                            writer.WriteLine(damage);
                        }
        ...
        public void DiscretizeCellsAroundIPs(Assembly assembly)
        {
            ...
            {
                int quad = QuadElements[i]; // Current quad element
                int NumIPs = assembly.ElementList[i].NumIPs; // Number of IPs per quad
                ...
                foreach (var array in XI)
                {
                    ...
                    double[] N = ShapeFunctions.EvaluateShapeFunctions(assembly.ElementList[quad - 1].Type, xi, eta);
                    Matrix NMat = ShapeFunctions.AssembleShapeFunctions(assembly.ElementList[quad - 1].Type, N);
                    double[] xy = NMat * assembly.ElementList[i].NodalLocations;
        ...
        private void InterpolateAtNodes(Assembly assembly)
        {
            ...
            for (int i = 0; i < NumElements; i++)
            {
                ...
                double[] q = Assembly.GlobalToLocalVector(assembly, Element, i, assembly.GlobalQ);
                ...
                for (int j = 0; j < NumNodes; j++)
                {
                    ...
                    Matrix NMat = ShapeFunctions.AssembleShapeFunctions(Element.Type, N);
                    double[] disp = NMat * q;

                    // Calculate B Matrix:
                    Matrix BMatrix = Elements.BMatrix2D(Element.Type, Element.NodalLocations, xi, eta);
                    ...
                    else if (Element.ModelID == 3)
                    {
                        ...
                        DMatrix = SmearedCrack.CalcDMatrixIsotropicDamage(Element.Model.E2, Element.Model.nu23, Element.PlaneStressPlaneStrain, 0.0);
                    }
        ...
        {
            // Inputs
            int DOFPNode = assembly.NDOFPNode;
            int NumElements = assembly.NumberOfElements;
            double[] Q = assembly.GlobalQ;
            List<int> QuadElements = this.QuadElements;
            List<int[]> ConnectMat = this.Connectivity;

            // Calculate displacements around for new cells
            int NumElemsPlusQuads = NumElements + QuadElements.Count;
            ...
            for (int i = 0; i < NumElemsPlusQuads; i++)
            {
                if (i < NumElements - 1)
                {
                    ...
                    for (int j = 0; j < NumNodes; j++)
                    {
                        int GlobalNode = ConnectMat[i][j];
                        int xDOF = DOFPNode * GlobalNode - 2;
                        int yDOF = DOFPNode * GlobalNode - 1;
                        this.NodalDisplacements[GlobalNode - 1][0] = Q[xDOF];
                        this.NodalDisplacements[GlobalNode - 1][1] = Q[yDOF];
                ...
                else if (i >= NumElements)
                {
                    ...
                    int NumIPs = assembly.ElementList[Quad - 1].NumIPs;
                    double[] q = Assembly.GlobalToLocalVector(assembly, assembly.ElementList[Quad - 1], Quad - 1, Q);
                    ...
                    {
                        RowCount++;
                        int RowNum = NumElements + RowCount;
                        for (int k = 0; k < Cells[j].Count; k++)
                        {
                            int NodeNum = ConnectMat[RowNum - 1][k];
        ...
        private void InterpolateQuadsForRVE(Assembly assembly, Elements Element)
        {
            ...
            string type = Element.Type;
            int NumNodes = Element.NumNodes;
            int ElemNum = Element.ElementNumber;
            double[] NodalLocations = Element.NodalLocations;
            List<int[]> ConnectMat = assembly.Connectivity;
            ...
            for (int i = 0; i < NumNodes; i++)
            {
                int NodeNum = (int)ConnectMat[ElemNum - 1][i]; // Global node number
                ...
                if (DataCheck == 0.0)
                {
                    ...
                    double[] N = ShapeFunctions.EvaluateShapeFunctions(Element.Type, xi, eta);
                    Matrix NMat = ShapeFunctions.AssembleShapeFunctions(Element.Type, N);
                    double[] disp = NMat * NodalLocations;
                    ...
                    if (i == 7)
                    {
                        NodeNum_L = ConnectMat[ElemNum - 1][6];
                        NodeNum_R = ConnectMat[ElemNum - 1][0];
                    }
                    else if (i == 0)
                    {
                        NodeNum_L = ConnectMat[ElemNum - 1][7];
                        NodeNum_R = ConnectMat[ElemNum - 1][1];
                    }
                    else
                    {
                        NodeNum_L = (int)ConnectMat[ElemNum - 1][i - 1];
                        NodeNum_R = (int)ConnectMat[ElemNum - 1][i + 1];
                    }
        ...
        /// </summary>
        private static Matrix CalcDMatrixIsotropic(double E, double nu, int PlaneStressPlaneStrain)
        {
            Matrix DMatrix = new(3, 3);
